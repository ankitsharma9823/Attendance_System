...
// const CMD_GET_TIME = 201;
// const CMD_SET_TIME = 202;
// const OPERATION_TIMEOUT_MS = parseInt(process.env.DEVICE_OP_TIMEOUT || "15000", 10);
...
// // ── withDevice — now with hard timeout ──────────────────────────
// const withDevice = async <T>(
//   fn: (zk: InstanceType<typeof ZKLib>) => Promise<T>,
//   label: string = "device operation",
// ): Promise<T> => {
//   // Pause real-time listener if active
//   if (realTimeZk && !isRealTimePaused) {
//     isRealTimePaused = true;
//     console.log(`[Device Engine] Pausing real-time listener for: ${label}`);
//     try { await realTimeZk.disconnect(); } catch (_) {}
//   }

//   if (currentDeviceMutex) {
//     console.log(`[Device Engine] Port busy, queuing: ${label}`);
//     await currentDeviceMutex.catch(() => {});
//   }

//   let resolveLock: () => void = () => {};
//   currentDeviceMutex = new Promise<void>((resolve) => { resolveLock = resolve; });
...
//   try {
//     await withTimeout(zk.createSocket(), DEVICE_CONFIG.TIMEOUT, "createSocket");
//     return await withTimeout(fn(zk), OPERATION_TIMEOUT_MS, label);
//   } catch (err) {
//     try {
//       if (zk.zklibTcp?.socket) zk.zklibTcp.socket.destroy();
//     } catch (_) {}
//     throw new Error(deviceErrorMessage(err));
//   } finally {
...
//   try {
//     const users = (await withDevice((zk) => zk.getUsers(), "syncEmployees")) as {
//       data?: Array<{ userId?: string; uid?: number; name?: string; role?: number }>;
//     };
//     const userList = users?.data ?? [];
...
// const runSyncOnce = async () => {
//   const logs = (await withDevice((zk) => zk.getAttendances(), "getAttendances")) as {
//     data?: DeviceLog[];
//   };
...
//   isRealTimePaused = true;
//   if (realTimeZk) {
//     try { realTimeZk.disconnect(); } catch (_) {}
//     realTimeZk = null;
//   }
// };
...
//   const retryDelay = Math.min(5000 * Math.pow(1.5, realTimeRetryCount), 60000);

//   if (realTimeRetryCount >= MAX_REALTIME_RETRIES) {
//     console.error("[Device Engine] Real-time listener gave up after max retries.");
//     console.error("[Device Engine] Use POST /api/device/sync for manual polling.");
//     realTimeRetryCount = 0;
//     return;
//   }

//   if (realTimeRetryCount > 0) {
//     console.log(`[Device Engine] Retry ${realTimeRetryCount}/${MAX_REALTIME_RETRIES} in ${retryDelay / 1000}s...`);
//   }

//   console.log("[Device Engine] Starting real-time listener...");
...
//   try {
//     await withTimeout(zk.createSocket(), DEVICE_CONFIG.TIMEOUT, "realtime-connect");
//     console.log("[Device Engine] Real-time socket connected.");
//     realTimeRetryCount = 0; // reset on successful connect
...
//       const txTime = parseHardwareTime(log);
//       console.log(`[Device Engine] Real-time punch: Employee ${empId} at ${formatWallClock(txTime)}`);
...
//         });
//       } catch (err: any) {
//         console.error(`[Device Engine] Punch processing failed for ${empId}:`, err.message);
//       }
//     });
...
//       zk.zklibTcp.socket.on("close", () => {
//         if (isRealTimePaused) return;
//         console.warn("[Device Engine] Socket closed — reconnecting...");
//         realTimeZk = null;
//         realTimeRetryCount++;
//         setTimeout(startRealTimeListener, retryDelay);
//       });

//       zk.zklibTcp.socket.on("error", (err: Error) => {
//         // close fires after error, just log here
//         console.error("[Device Engine] Socket error:", err.message);
//       });
//     }

//   } catch (err: any) {
//     console.error("[Device Engine] Failed to connect real-time listener:", err.message);
//     realTimeZk = null;
//     if (!isRealTimePaused) {
//       realTimeRetryCount++;
//       setTimeout(startRealTimeListener, retryDelay);
