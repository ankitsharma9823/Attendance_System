...
// const withDevice = async <T>(fn: (zk: any) => Promise<T>): Promise<T> => {
//   const zk = new ZKLib(
//     DEVICE_CONFIG.IP,
//     DEVICE_CONFIG.PORT,
...
// const encodeZKTime = (date: Date): Buffer => {
...
// const decodeZKTime = (buf: Buffer): Date => {
...
// export const setDeviceTime = async (date?: Date): Promise<void> => {
...
// export const deleteDeviceUser = async (uid: number): Promise<void> => {
//   await withDevice(async (zk) => {
...
// export const disableDevice = async (): Promise<void> => {
//   await withDevice(async (zk) => {
//     await zk.disableDevice();
...
// export const enableDevice = async (): Promise<void> => {
//   await withDevice(async (zk) => {
//     await zk.enableDevice();
...
const decodeZKTime = (buf: Buffer): Date => {
  const seconds = buf.readUInt32LE(0);
  const decoded = new Date(ZK_EPOCH + seconds * 1000);
  console.log(`[Device] Decoding: ${seconds}s → ${decoded.toLocaleString()}`);
  return decoded;
};
...
export const setDeviceTime = async (date?: Date): Promise<void> => {
  await withDevice(async (zk) => {
    ...
    // FIX: Removed disableDevice/enableDevice — they cause TIMEOUT_ON_WRITING_MESSAGE
    // on many ZKTeco firmware versions before the connection is fully ready
    const buf = encodeZKTime(localDate);
    console.log(`[Device] Sending hex: ${buf.toString("hex")}`);
    await zk.executeCmd(CMD_SET_TIME, buf);
