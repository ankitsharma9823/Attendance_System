...
export const updateDeviceUser = async (
  ...
  return withDevice(async (zk) => {
    ...
    await prisma.employee
      ...
      .catch((dbErr) => console.error("[Device Engine] DB desync on user update:", dbErr?.message ?? dbErr));
    console.log(
...
export const startRealTimeListener = async () => {
  ...
  if (realTimeRetryCount >= MAX_REALTIME_RETRIES) {
    console.error(`[Device Engine] Real-time listener gave up after ${MAX_REALTIME_RETRIES} retries. Use POST /api/device/sync for manual polling.`);
  ...
  try {
    ...
    if (zk.zklibTcp && zk.zklibTcp.socket) {
      zk.zklibTcp.socket.on("close", () => {
        if (!isRealTimePaused) {
          console.warn("[Device Engine] Real-time socket closed unexpectedly. Reconnecting...");
          realTimeZk = null;
          realTimeRetryCount++;
    ...
    if (!isRealTimePaused) {
      realTimeRetryCount++;
      console.log(`[Device Engine] Retrying real-time connection in ${retryDelay / 1000}s...`);
      setTimeout(startRealTimeListener, retryDelay);
    }
  }
};
